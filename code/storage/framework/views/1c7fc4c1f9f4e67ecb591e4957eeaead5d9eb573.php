        ...
        <div class="card-body">
            ...
            <form action="<?php echo e(route('highlights.store')); ?>" method="POST" enctype="multipart/form-data">
                ...
                <div class="mb-3">
                    ...
                    <input type="text" name="title_en" class="form-control border rounded p-2" value="<?php echo e(old('title_en')); ?>" required>
                </div>

                <div class="mb-3">
                    <label>Title (TH)</label>
                    <input type="text" name="title_th" class="form-control border rounded p-2" value="<?php echo e(old('title_th')); ?>" required>
                </div>

                <div class="mb-3">
                    <label>Description (EN)</label>
                    <textarea name="description_en" class="form-control border rounded p-2"><?php echo e(old('description_en')); ?></textarea>
                </div>

                <div class="mb-3">
                    <label>Description (TH)</label>
                    <textarea name="description_th" class="form-control border rounded p-2"><?php echo e(old('description_th')); ?></textarea>
                </div>
                ...
                <div class="mb-3">
                    ...
                    <input type="number" name="priority" class="form-control border rounded" value="<?php echo e(old('priority', 1)); ?>" required>
                </div>
                        ...
                        <div class="modal-body">
                            <?php if(!empty($existingTags) && is_iterable($existingTags)): ?>
                                <?php $__currentLoopData = $existingTags; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $tag): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                    <button type="button" class="btn btn-outline-primary tag-option m-1" data-id="<?php echo e(is_object($tag) ? $tag->id : $tag); ?>">
...
$(document).ready(function() {
    $('.tag-option').click(function() {
        ...
        if ($('#selected-tags input[value="' + tagId + '"]').length == 0) {
            $('#selected-tags').append(`
                <span class="badge badge-primary m-1">${tag}
                    <span class="remove-tag" data-id="${tagId}" style="cursor:pointer">&times;</span>
                ...
                if (response.success) {
                    $('#selected-tags').append(`
                        <span class="badge badge-primary m-1">${response.tag.name}
                            <span class="remove-tag" data-id="${response.tag.id}" style="cursor:pointer">&times;</span>
    ...
    $(document).on('click', '.remove-tag', function(){
        var tagElement = $(this).parent('.badge');
        var tagId = $(this).data('id');
        if (confirm('Are you sure you want to delete this tag?')) {
            $.ajax({
                url: "<?php echo e(route('tags.destroy', '')); ?>/" + tagId,
                method: "DELETE",
                data: {
                    _token: "<?php echo e(csrf_token()); ?>"
                },
...
<?php echo $__env->make('dashboards.users.layouts.user-dash-layout', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH C:\xampp\htdocs\git-group-repository-group-1\code\resources\views/highlights/create.blade.php ENDPATH**/ ?>