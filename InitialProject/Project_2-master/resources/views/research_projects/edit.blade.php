...
<div class="container">
    ...
    <div class="card col-md-12" style="padding: 16px;">
        <div class="card-body">
            <h4 class="card-title">{{ trans('message.editResearch')}}</h4>
            <p class="card-description">{{ trans('message.fillInfoResearch')}}</p>
            <form action="{{ route('researchProjects.update',$researchProject->id) }}" method="POST">
                @csrf
                @method('PUT')
                <div class="form-group row">
                    <p class="col-sm-3 "><b>{{ trans('message.researchName')}}</b></p>
                    <div class="col-sm-8">
                ...
                <div class="form-group row mt-2">
                    <p class="col-sm-3 "><b>{{ trans('message.yearSubmission')}}</b></p>
                    <div class="col-sm-8">
                ...
                <div class="form-group row mt-2">
                    ...
                    <div class="col-sm-3">
                        <select id='dep' style='width: 200px;' class="custom-select my-select"  name="responsible_department">
                            <option value=''>{{ trans('message.department')}}</option>@foreach($deps as $dep)<option value="{{ $dep->department_name_th }}" {{ $dep->department_name_th == $researchProject->responsible_department ? 'selected' : '' }}>{{ $dep->department_name_th }}</option>@endforeach
                        </select>
                    </div>
                </div>
                
                <div class="form-group row">
                    <p class="col-sm-3 "><b>{{ trans('message.researchDetail')}}</b></p>
                    <div class="col-sm-8">
                ...
                <div class="form-group row">
                    ...
                    <div class="col-sm-8">
                        <select id='status' class="custom-select my-select" style='width: 200px;' name="status">
                            <option value="1" {{ 1 == $researchProject->status ? 'selected' : '' }}>{{ trans('message.applyFor')}}</option>
                            <option value="2" {{ 2 == $researchProject->status ? 'selected' : '' }}>{{ trans('message.process')}}</option>
                            <option value="3" {{ 3 == $researchProject->status ? 'selected' : '' }}>{{ trans('message.endProject')}}</option>
                        </select>
                    </div>
                </div>
                ...
                <div class="col-xs-12 col-sm-12 col-md-12 mb-3">
                    <table class="table " id="dynamicAddRemove">
                        <tr>
                            <th width="522.98px">{{ trans('message.projectmanagerCoInternal')}}</th>
                            <th><button type="button" name="add" id="add-btn2" class="btn btn-success btn-sm add"><i class="mdi mdi-plus"></i></button></th>
                        </tr>
                    </table>
                </div>
                <div class="form-group row">
                        <label for="exampleInputpaper_author" class="col-sm-3 col-form-label">{{ trans('message.projectmanagerCoExternal')}}</label>
                        <div class="col-sm-9">
                ...
                <a class="btn btn-light mt-5" href="{{ route('researchProjects.index') }}">{{ trans('message.back')}}</a>
            </form>
        </div>
    </div>
</div>
