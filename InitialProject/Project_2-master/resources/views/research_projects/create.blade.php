...
<div class="container">
    ...
    <div class="col-md-12 grid-margin stretch-card">
        <div class="card" style="padding: 16px;">
            <div class="card-body">
                <h4 class="card-title">{{ trans('message.createResearchProject') }}</h4>
                <p class="card-description">{{ trans('message.createResaerchProjectDetail') }}</p>
                <form action="{{ route('researchProjects.store') }}" method="POST">
                    ...
                    <div class="form-group row mt-2">
                        <label for="exampleInputfund_details" class="col-sm-2 ">{{ trans('message.selectScholarship') }}</label>
                        <div class="col-sm-4">
                            <select id='fund' style='width: 200px;' class="custom-select my-select" name="fund">
                                <option value='' disabled selected>{{ trans('message.selectScholarship') }}</option>@foreach($funds as $fund)<option value="{{ $fund->id }}">{{ $fund->fund_name }}</option>@endforeach
                            </select>
                        </div>
                    </div>
                    <div class="form-group row">
                        <label for="exampleInputproject_year" class="col-sm-2 ">{{ trans('message.yearSubmission') }} (ค.ศ.)</label>
                        <div class="col-sm-4">
                    ...
                    <div class="form-group row mt-2">
                        ...
                        <div class="col-sm-4">
                            <input type="int" name="budget" class="form-control" placeholder="{{  trans('message.unitBaht') }}" value="{{ old('budget') }}">
                        </div>
                    </div>
                    <div class="form-group row mt-2">
                        ...
                        <div class="col-sm-9">
                            <select id='dep' style='width: 200px;' class="custom-select my-select" name="responsible_department">
                                @if (app()->getLocale() == 'en')
                                <option value='' disabled selected>{{ trans('message.selectMajor') }}</option>@foreach($deps as $dep)<option value="{{ $dep->department_name_en }}">{{ $dep->department_name_en }}</option>@endforeach
                                @else
                                <option value='' disabled selected>{{ trans('message.selectMajor') }}</option>@foreach($deps as $dep)<option value="{{ $dep->department_name_th }}">{{ $dep->department_name_th }}</option>@endforeach
                                @endif
                            </select>
                        </div>
                    </div>
                    <div class="form-group row mt-2">
                        <label for="exampleInputfund_details" class="col-sm-2 ">{{ trans('message.projectDetail') }}</label>
                        <div class="col-sm-9">
                            <textarea type="text" name="note" class="form-control form-control-lg" style="height:150px" value="{{ old('note') }}"></textarea>
                        </div>
                    </div>
                    <div class="form-group row mt-2">
                        ...
                        <div class="col-sm-3">
                            <select id='status' class="custom-select my-select" name="status">
                                <option value="" disabled selected>{{ trans('message.specifyOperationStatus') }}</option>
                                <option value="1">{{ trans('message.apply') }}</option>
                                <option value="2">{{ trans('message.proceed') }}</option>
                                <option value="3">{{ trans('message.closeProject') }}</option>
                            </select>
                        </div>
                    </div>
                    <!-- <div class="form-group row">
                    ...
                    <div class="form-group row mt-2">
                        ...
                        <div class="col-sm-9">
                            <select id='head0' style='width: 200px;' name="head">
                                @if (app()->getLocale() == 'en')
                                <option value=''>{{ trans('message.selectUser') }}</option>@foreach($users as $user)<option value="{{ $user->id }}">{{ $user->fname_en }} {{ $user->lname_en }}</option>
                                @endforeach
                                @else
                                <option value=''>{{ trans('message.selectUser') }}</option>@foreach($users as $user)<option value="{{ $user->id }}">{{ $user->fname_th }} {{ $user->lname_th }}</option>
                                @endforeach
                                @endif
                            </select>
                        </div>
                    ...
                    <div class="form-group row mt-2">
                        <label for="exampleInputfund_details" class="col-sm-2 ">{{ trans('message.projectmanagerCoInternal') }}</label>
                        <div class="col-sm-9">
                    ...
                    <div class="form-group row mt-2">
                        <label for="exampleInputpaper_doi" class="col-sm-2 ">{{ trans('message.projectmanagerCoExternal') }}</label>
                        <div class="col-sm-9">
                            <div class="table-responsive">
                                <table class="table table-hover small-text" id="tb">
                                    <tr class="tr-header">
                                        <th>{{ trans('message.positionOrTitle') }}</th>
                                        <th>{{ trans('message.name') }}</th>
                                        <th>{{ trans('message.surname') }}</th>
                                        <!-- <th>Email Id</th> -->
                                    ...
                                    <tr>
                                        <td><input type="text" name="title_name[]" class="form-control" placeholder="{{ trans('message.positionOrTitle') }}"></td>
                                        <td><input type="text" name="fname[]" class="form-control" placeholder="{{ trans('message.name') }}" ></td>
                                        <td><input type="text" name="lname[]" class="form-control" placeholder="{{ trans('message.surname') }}" ></td>
                    ...
                    <div class="pt-4">
                        ...
                        <a class="btn btn-light" href="{{ route('researchProjects.index')}}">{{ trans('message.back') }}</a>
                    </div>
                </form>
            </div>
        </div>
